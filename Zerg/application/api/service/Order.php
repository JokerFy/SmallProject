...
class Order
{
    protected $oProducts; //客户端传来的订单中的商品列表
    ...
    public function place($uid, $oProducts)
    {
        ...
        $this->products = $this->getProductsByOrder($oProducts);//根据传递来的订单号查询数据表中获取所有订单上的商品信息
        $this->uid = $uid;
...
//            ->update(['status' => OrderStatusEnum::DELIVERED]);
    ...
    private function getOrderStatus() //订单的状态由每个商品决定，只要有一个商品不通过则失败
    {
        $status = [
            'pass' => true,
            'orderPrice' => 0,
    ...
    private function getProductStatus($oPID, $oCount, $products)
    {
        ...
        for ($i = 0; $i < count($products); $i++) { //确定订单中一类商品是否存在于获取的商品列表中
